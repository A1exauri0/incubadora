...
@section('administrar', 'Tipos')

<!-- ¿Qué se va a administrar en singular? -->
@section('administrar_s', 'Tipo')
        ...
        $(document).ready(function() {
            ...
            function actualizarNumerosControlSeleccionados() {
                ...
                $('table tbody input[type="checkbox"]:checked').each(function() {
                    ...
                    if (!document.getElementById('idTipo_eliminar_' + $(this).val())) {
                        var input = document.createElement('input');
                        input.type = 'hidden';
                        input.name = 'idTipo_eliminar_' + $(this).val();
                        input.id = 'idTipo_eliminar_' + $(this).val();
                ...
                $('table tbody input[type="checkbox"]:not(:checked)').each(function() {
                    if (document.getElementById('idTipo_eliminar_' + $(this).val())) {
                        document.getElementById('idTipo_eliminar_' + $(this).val()).remove();
                        // console.log('Se eliminó el input: ' + 'idTipo_eliminar_' + $(this).val());
                    }
...
@section('script_validacion_campos')
    <script>
        document.querySelectorAll('.idTipo').forEach(function(element) {
...
@section('datos')
    ...
    @else
        @php
            $tipos = $tipos->toArray(); // Convert $tipos to an array
            ...
            $tipos_pagina = array_slice($tipos, $inicio, $segmento); // Obtener los tipos de la página actual
        @endphp

        @foreach ($tipos_pagina as $tipo)
            <tr>
                <td>
                    <span class="custom-checkbox">
                        <input type="checkbox" class="checkbox" name="options[]" value="{{ $tipo->idTipo }}">
                        <label></label>
                    </span>
                </td>
                <td class="idTipo">{{ $tipo->idTipo }}</td>
                <td class="nombre">{{ $tipo->nombre }}</td>
                <td>
                    <a href="#editEmployeeModal" class="edit" data-toggle="modal"
                        data-idtipo="{{ $tipo->idTipo }}" data-nombre="{{ $tipo->nombre }}">
                        <i class="material-icons" data-toggle="tooltip" title="Editar">&#xE254;</i></a>
                    <a href="#deleteEmployeeModal" class="delete" data-toggle="modal"
                        data-idtipo="{{ $tipo->idTipo }}"><i class="material-icons" data-toggle="tooltip"
                            title="Eliminar">&#xE872;</i></a>
                </td>
            </tr>
        @endforeach
        ...
        @php
            $totalPaginas = ceil(count($tipos) / $segmento); // Total de páginas
        @endphp
...
@section('modulo_agregar')
    ...
    <form action="{{ route('tipos.agregar') }}" method="POST">
...
@section('modulo_editar')
    ...
    @if ($tipos)
        <form action="{{ route('tipos.editar') }}" method="POST">
            {{-- Muy importante la directiva siguiente. --}}
            @csrf

            {{-- Aquí se debe guardar el numero de control a enviar al método del controlador para eliminar. --}}
            <input type="hidden" name="idTipo_editar" id="idTipo_editar">
        ...
        {{-- Escucha que identifica según el atributo data-idTipo el no. control que se va a editar. --}}
        <script>
...
@section('modulo_eliminar')
    ...
    <form action="{{ route('tipos.eliminar') }}" method="POST">
        {{-- Muy importante la directiva siguiente. --}}
        @csrf

        {{-- Aquí se debe guardar el numero de control a enviar al método del controlador para eliminar. --}}
        <input type="hidden" name="idTipo_eliminar" id="idTipo_eliminar">
    ...
    {{-- Escucha que identifica según el atributo data-idTipo el no. control que se va a eliminar. --}}
    <script>
        $(document).ready(function() {
            $('a.delete').click(function() {
                var noControlEliminar = $(this).data('idtipo');
                $('#idTipo_eliminar').val(noControlEliminar);
...
@section('modulo_eliminar_multiple')
    <form id="formulario_eliminar_multiple" action="{{ route('tipos.eliminarMultiple') }}" method="POST">
