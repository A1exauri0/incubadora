...
@section('administrar', 'Asesores')

<!-- ¿Qué se va a administrar en singular? -->
@section('administrar_s', 'Asesor')
...
<script>
    $(document).ready(function() {
        ...
        var idAsesorsSeleccionadas = [];

        // Función para actualizar el arreglo cuando se selecciona o deselecciona un checkbox
        function actualizaridAsesorsSeleccionadas() {
            idAsesorsSeleccionadas = [];
            $('table tbody input[type="checkbox"]:checked').each(function() {
                idAsesorsSeleccionadas.push($(this).val());

                // Si se selecciona un checkbox, se agrega un input oculto al formulario
                if (!document.getElementById('idAsesor_eliminar_' + $(this).val())) {
                    var input = document.createElement('input');
                    input.type = 'hidden';
                    input.name = 'idAsesor_eliminar_' + $(this).val();
                    input.id = 'idAsesor_eliminar_' + $(this).val();
            ...
            $('table tbody input[type="checkbox"]:not(:checked)').each(function() {
                if (document.getElementById('idAsesor_eliminar_' + $(this).val())) {
                    document.getElementById('idAsesor_eliminar_' + $(this).val()).remove();
                    // console.log('Se eliminó el input: ' + 'idAsesor_eliminar_' + $(this).val());
                }

                // console.log('Numeros de control seleccionados: ' + idAsesorsSeleccionadas);
            });

            boton_eliminar = document.getElementById('btnEliminar');
            //console.log("botoneliminar: ", boton_eliminar);

            if (idAsesorsSeleccionadas.length === 0) { //Si está vacío el botón se inhabilita
        ...
        $("#selectAll").click(function() {
            ...
            actualizaridAsesorsSeleccionadas();
        });
...
<script>
    document.querySelectorAll('.idAsesor').forEach(function(element) {
...
@else
@php
$asesores = $asesores->toArray(); // Convertir $asesores a un array
...
$asesores_pagina = array_slice($asesores, $inicio, $segmento); // Obtener las asesores de la página actual
@endphp

@foreach ($asesores_pagina as $asesor)
<tr>
    <td>
        <span class="custom-checkbox">
            <input type="checkbox" class="checkbox" name="options[]" value="{{ $asesor->idAsesor }}">
            <label></label>
        </span>
    </td>
    <td class="idAsesor">{{ $asesor->idAsesor }}</td>
    <td class="nombre">{{ $asesor->nombre }}</td>
    <td class="telefono">{{ $asesor->telefono }}</td>
    <td class="correo">{{ $asesor->correo_electronico }}</td>
    <td>{{ $asesor->fecha_agregado }}</td>
    <td>
        <a href="#editEmployeeModal" class="edit" data-toggle="modal"
            data-idasesor="{{ $asesor->idAsesor }}" data-nombre="{{ $asesor->nombre }}" data-telefono="{{ $asesor->telefono }}" data-correo="{{ $asesor->correo_electronico }}">
            <i class="material-icons" data-toggle="tooltip" title="Editar">&#xE254;</i></a>
        <a href="#deleteEmployeeModal" class="delete" data-toggle="modal"
            data-idasesor="{{ $asesor->idAsesor }}"><i class="material-icons" data-toggle="tooltip"
                title="Eliminar">&#xE872;</i></a>
    </td>
</tr>
@endforeach
...
@php
$totalPaginas = ceil(count($asesores) / $segmento); // Total de páginas
@endphp
...
<form action="{{ route('asesores.agregar') }}" method="POST">
    ...
    <div class="modal-body">
        ...
        <div class="form-group">
            ...
            <input id="telefono_agregar" name="telefono_agregar" type="tel" class="form-control telefono" maxlength="10" required>
        </div>
        <div class="form-group">
            <label>Correo electrónico</label>
            <input id="correo_agregar" name="correo_agregar" type="email" class="form-control correo" maxlength="50" required>
        </div>
    </div>
...
@if ($asesores)
<form action="{{ route('asesores.editar') }}" method="POST">
    {{-- Muy importante la directiva siguiente. --}}
    @csrf

    {{-- Aquí se debe guardar el numero de control a enviar al método del controlador para eliminar. --}}
    <input type="hidden" name="idAsesor_editar" id="idAsesor_editar">
    ...
    <div class="modal-body">
        ...
        <div class="form-group">
            <label>Telefono</label>
            <input id="telefono_campo" type="tel" class="form-control telefono ceditar" maxlength="10" required>
        </div>
        <div class="form-group">
            <label>Correo electrónico</label>
            <input id="correo_campo" type="email" class="form-control correo ceditar" maxlength="50" required>
        </div>
    </div>

    <div class="modal-footer">
        <input type="button" class="btn btn-default" data-dismiss="modal" value="Cancelar" id="CancelarEditar">
        <input type="submit" class="btn btn-info" value="Guardar">
    </div>
</form>

{{-- Escucha que identifica según el atributo data-idAsesor el no. control que se va a editar. --}}
<script>
    // Se ejecuta cuando el documento esté listo
    $(document).ready(function() {
        $('a.edit').click(function() {
            // Se obtienen los datos del registro a editar
            var idAsesor = $(this).data('idasesor');
            var nombre = $(this).data('nombre');
            var telefono = $(this).data('telefono');
            var correo = $(this).data('correo');

            $('#idAsesor_eliminar').val(idAsesor);
            ...
            //console.log("No control:", idAsesor);
        });
    });
...
<form action="{{ route('asesores.eliminar') }}" method="POST">
    {{-- Muy importante la directiva siguiente. --}}
    @csrf
...
{{-- Escucha que identifica según el atributo data-idAsesor el no. control que se va a eliminar. --}}
<script>
    $(document).ready(function() {
        $('a.delete').click(function() {
            ...
            //console.log("No control:", idAsesorEliminar);
        });
    });
</script>
...
<form id="formulario_eliminar_multiple" action="{{ route('asesores.eliminarMultiple') }}" method="POST">
